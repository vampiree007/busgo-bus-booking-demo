...
// Pick a writable dir for the demo data store. Locally we use ./.data so
// bookings persist across restarts. On read-only serverless filesystems
// (e.g. Netlify), that path isn't writable, so fall back to the OS temp dir —
// bookings work but are ephemeral, which is fine for a demo.
function pickDataDir() {
  ...
  try {
    ...
    const fallback = path.join(os.tmpdir(), 'busgo-data');
